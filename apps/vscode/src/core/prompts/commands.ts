...
export const bmsAutosarToolResponse = () =>
	`<explicit_instructions type="bms-autosar">
The user has explicitly asked you to generate or work on AUTOSAR Classic Platform software artifacts for a Battery Management System (BMS). You MUST use the built-in bms-autosar knowledge base to guide generation.

Follow this workflow:
1. If the user's request is vague, ask concise clarifying questions about the component type, name, ports, runnables, and target output format.
2. Load the bms-autosar skill by calling the use_skill tool with skill_name="bms-autosar" so the full knowledge base is active.
3. Based on the loaded skill and the user's requirements, call the bms_autosar_generate tool with the appropriate parameters to produce the requested artifact(s).

The bms_autosar_generate tool can produce:
- SWC implementation (.c/.h) + ARXML descriptor
- BSW module implementation (.c/.h) + configuration files
- RTE interface definition
- Standalone ARXML descriptor
- BMS domain components: "bms_csc", "bms_controller", "bms_balancer", "bms_thermal_manager", "bms_charger", "bms_diagnosis"

Parameters:
- component_type: (optional) One of "swc", "bsw_module", "rte_interface", "arxml_descriptor", "service", "ecu_extract", "bms_csc", "bms_controller", "bms_balancer", "bms_thermal_manager", "bms_charger", "bms_diagnosis". Inferred from requirements if omitted.
- component_name: (required) The SHORT-NAME of the component (e.g., "BmsStateEstimator").
- ports: (optional) JSON array of port definitions, each with name, interface_type ("S/R" or "C/S"), direction ("provided" or "required"), and data_type.
- runnables: (optional) JSON array of runnable names and triggering events (e.g., [{"name":"Run100ms","event":"TimingEvent","period_ms":100}]).
- requirements: (optional) Free-form extra requirements or constraints.
- output_format: (optional) "c_code", "arxml", or "both". Defaults to "both".

Usage:
<bms_autosar_generate>
<component_type>swc</component_type>
<component_name>BmsStateEstimator</component_name>
<ports>[{"name":"CellVoltage","interface_type":"S/R","direction":"required","data_type":"Adc_VoltageType"},{"name":"StateOfCharge","interface_type":"S/R","direction":"provided","data_type":"Percent_Type"}]</ports>
<runnables>[{"name":"Run100ms","event":"TimingEvent","period_ms":100}]</runnables>
<requirements>Estimate SOC using extended Kalman filter, support SOH tracking</requirements>
<output_format>both</output_format>
</bms_autosar_generate>

If the user has only provided high-level intent (e.g., "create a BMS state estimator"), infer sensible defaults for ports and runnables, state them briefly, and then call the tool.
Below is the user's input when they indicated they wanted BMS AUTOSAR generation.
</explicit_instructions>\n`
