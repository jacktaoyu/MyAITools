...
describe("SubagentBuilder", () => {
	...
	it("uses cached config by subagent name and applies act-mode provider model override", async () => {
		...
		const buildApiHandlerStub = sinon.stub(api, "buildApiHandler").resolves(fakeHandler as never)

		const builder = new SubagentBuilder(createTaskConfig("act", "openai"), "cached-agent")
		await builder.initializeApi()
	...
	it("uses defaults when no cached config is provided", async () => {
		sinon.stub(AgentConfigLoader, "getInstance").returns({
			getCachedConfig: () => undefined,
		} as unknown as AgentConfigLoader)

		sinon.stub(api, "buildApiHandler").resolves({ getModel: sinon.stub(), createMessage: sinon.stub() } as never)
	...
	it("applies plan-mode openrouter model override fields", async () => {
		sinon.stub(AgentConfigLoader, "getInstance").returns({
			getCachedConfig: (subagentName?: string) =>
		...
		const buildApiHandlerStub = sinon.stub(api, "buildApiHandler").resolves({
			getModel: sinon.stub(),
			createMessage: sinon.stub(),
		} as never)

		const builder = new SubagentBuilder(createTaskConfig("plan", "openrouter"), "openrouter-agent")
		await builder.initializeApi()
	...
	it("builds native tools by filtering allowed ids and context requirements then converting", async () => {
		sinon.stub(AgentConfigLoader, "getInstance").returns({
			getCachedConfig: (subagentName?: string) =>
