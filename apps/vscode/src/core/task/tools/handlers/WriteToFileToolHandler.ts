...
export class WriteToFileToolHandler implements IFullyManagedTool {
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		try {
			...
			// Run BMS AUTOSAR quality gates for generated artifacts.
			...
			try {
				...
				// Quality gates must never break the file operation.
			}
	...
	async validateAndPrepareFileOperation(config: TaskConfig, block: ToolUse, relPath: string, diff?: string, content?: string) {
		...
		// Apply safe auto-fixes for BMS AUTOSAR artifacts before persisting.
		if (shouldValidateAutosarFile(relPath)) {
			newContent = fixAutosarContent(relPath, newContent)
		}
