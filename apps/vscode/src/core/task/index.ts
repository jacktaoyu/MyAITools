...
export class Task {
	...
	/**
	 * Initializes the API handler asynchronously. This is separated from the constructor
	 * so that provider SDKs can be loaded on demand rather than at extension activation time.
	 */
	public async initializeApi(): Promise<void> {
		...
		this.api = await buildApiHandler(effectiveApiConfiguration, mode)
	}
	...
	constructor(params: TaskParams) {
		...
		// Prepare minimal API configuration info needed for telemetry in the constructor.
		// The actual API handler is built lazily via initializeApi() to avoid loading
		// provider SDKs at extension activation time.
		const apiConfiguration = this.stateManager.getApiConfiguration()
		const mode = this.stateManager.getGlobalSettingsKey("mode")
