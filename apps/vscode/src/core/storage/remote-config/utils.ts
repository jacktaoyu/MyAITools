...
import { REMOTE_CONFIG_OTEL_PROVIDER_ID } from "./constants"
	...
	try {
		...
		if (isOpenTelemetryConfigValid(otelConfig)) {
			const [{ OpenTelemetryClientProvider }, { OpenTelemetryTelemetryProvider }] = await Promise.all([
				import("@/services/telemetry/providers/opentelemetry/OpenTelemetryClientProvider"),
				import("@/services/telemetry/providers/opentelemetry/OpenTelemetryTelemetryProvider"),
			])
