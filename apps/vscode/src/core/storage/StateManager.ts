		...
		try {
			...
			const syncTaskHistoryFromDisk = async () => {
				try {
					...
					// Use SHA-256 hashes instead of comparing the JSON strings directly to avoid
					// allocating large intermediate strings for long task histories.
					const onDiskHash = createHash("sha256").update(JSON.stringify(onDisk)).digest("hex")
					const cachedHash = createHash("sha256").update(JSON.stringify(cached)).digest("hex")
					if (onDiskHash !== cachedHash) {
