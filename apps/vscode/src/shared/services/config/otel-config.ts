...
/**
 * Parse a comma-separated list of key=value pairs into a record.
 * Replaces the OpenTelemetry core helper so this config module can be loaded
 * without pulling in the entire @opentelemetry/core package at startup.
 */
function parseKeyPairsIntoRecord(input: string): Record<string, string> {
	...
	for (const pair of input.split(",")) {
		...
		if (key) {
			result[key] = rawValueParts.join("=").trim()
		}
	}
	return result
}
