...
export async function activate(context: vscode.ExtensionContext) {
	...
	*/
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(commands.BmsAutosarGenerate, async () => {
			const webview = WebviewProvider.getInstance() as VscodeWebviewProvider
			const webviewView = webview.getWebview()
			if (webviewView) {
				webviewView.show(true)
			...
			await sendAddToInputEvent("/bms-autosar ")
			telemetryService.captureButtonClick("command_bmsAutosarGenerate", webview.controller?.task?.ulid)
		}),
	)

	context.subscriptions.push(
		vscode.commands.registerCommand(commands.BmsAutosarQualityReport, async () => {
			const webview = WebviewProvider.getInstance() as VscodeWebviewProvider
			const webviewView = webview.getWebview()
			if (webviewView) {
				webviewView.show(true)
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(commands.BmsAutosarKnowledgeGraph, async () => {
			const webview = WebviewProvider.getInstance() as VscodeWebviewProvider
			const webviewView = webview.getWebview()
			if (webviewView) {
				webviewView.show(true)
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(commands.BmsAutosarOpenCompile, async () => {
			const webview = WebviewProvider.getInstance() as VscodeWebviewProvider
			const webviewView = webview.getWebview()
			if (webviewView) {
				webviewView.show(true)
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(commands.BmsAutosarOpenGenerator, async () => {
			const webview = WebviewProvider.getInstance() as VscodeWebviewProvider
			const webviewView = webview.getWebview()
			if (webviewView) {
				webviewView.show(true)
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(commands.BmsAutosarDashboard, async () => {
			const webview = WebviewProvider.getInstance() as VscodeWebviewProvider
			const webviewView = webview.getWebview()
			if (webviewView) {
				webviewView.show(true)
