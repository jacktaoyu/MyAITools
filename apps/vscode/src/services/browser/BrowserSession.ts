...
// Minimal type for puppeteer console messages so we don't need to import
// the non-exported LoggerMessage type from puppeteer-core at module load time.
type LoggerMessage = {
	type(): string
	text(): string
}
...
export class BrowserSession {
	...
	async doAction(action: (page: Page) => Promise<void>): Promise<BrowserActionResult> {
		...
		const LoggerListener = (msg: unknown) => {
			const logMsg = msg as LoggerMessage
			if (logMsg.type() === "log") {
				logs.push(logMsg.text())
			} else {
				logs.push(`[${logMsg.type()}] ${logMsg.text()}`)
			}
			lastLogTs = Date.now()
		}
