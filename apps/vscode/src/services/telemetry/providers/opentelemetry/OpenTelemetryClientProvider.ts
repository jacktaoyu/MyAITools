...
export class OpenTelemetryClientProvider {
	meterProvider: MeterProvider | null = null
	loggerProvider: LoggerProvider | null = null
	private readonly config: OpenTelemetryClientValidConfig | null

	/**
	 * Factory method so that async exporter creation can be awaited.
	 */
	static async create(config: OpenTelemetryClientValidConfig): Promise<OpenTelemetryClientProvider> {
		const provider = new OpenTelemetryClientProvider(config)
		...
		if (provider.config?.metricsExporter) {
			provider.meterProvider = await provider.createMeterProvider(resource)
		}

		// Initialize logs if configured
		if (provider.config?.logsExporter) {
			provider.loggerProvider = await provider.createLoggerProvider(resource)
		}
	...
	private constructor(config: OpenTelemetryClientValidConfig) {
		this.config = config
	...
	private async createMeterProvider(resource: Resource): Promise<MeterProvider> {
			...
			try {
				switch (exporterType) {
					...
					case "otlp": {
						...
						if (endpoint) {
							const reader = await createOTLPMetricReader(protocol, endpoint, insecure, interval, timeout, headers)
							if (reader) {
								readers.push(reader)
	...
	private async createLoggerProvider(resource: Resource): Promise<LoggerProvider> {
			...
			try {
				...
				switch (exporterType) {
					...
					case "otlp": {
						...
						if (endpoint) {
							exporter = await createOTLPLogExporter(protocol, endpoint, insecure, headers)
							if (exporter) {
