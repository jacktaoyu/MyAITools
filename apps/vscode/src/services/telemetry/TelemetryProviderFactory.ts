	...
	private static async createProvider(config: TelemetryProviderConfig): Promise<ITelemetryProvider> {
		switch (config.type) {
			...
			case "opentelemetry": {
				...
				// Lazy-load OpenTelemetry providers so the SDKs are only bundled/executed
				// when telemetry is actually enabled.
				const [{ OpenTelemetryClientProvider }, { OpenTelemetryTelemetryProvider }] = await Promise.all([
					import("./providers/opentelemetry/OpenTelemetryClientProvider"),
					import("./providers/opentelemetry/OpenTelemetryTelemetryProvider"),
				])
				const client = await OpenTelemetryClientProvider.create(otelConfig)
