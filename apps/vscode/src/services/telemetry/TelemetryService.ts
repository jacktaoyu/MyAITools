...
export class TelemetryService {
	...
	public static readonly METRICS = {
		...
		BMS_AUTOSAR: {
			GENERATE_DURATION_MS: "cline.bms_autosar.generate.duration_ms",
			KNOWLEDGE_REFRESH_DURATION_MS: "cline.bms_autosar.knowledge_refresh.duration_ms",
			EMBEDDING_CACHE_HIT_TOTAL: "cline.bms_autosar.embedding_cache.hit.total",
			EMBEDDING_CACHE_MISS_TOTAL: "cline.bms_autosar.embedding_cache.miss.total",
			RETRIEVAL_TFIDF_FALLBACK_TOTAL: "cline.bms_autosar.retrieval.tfidf_fallback.total",
			QUALITY_GATE_ISSUE_TOTAL: "cline.bms_autosar.quality_gate.issue.total",
		},
	}
	...
	private static readonly EVENTS = {
		...
		BMS_AUTOSAR: {
			GENERATE_STARTED: "bms_autosar.generate.started",
			GENERATE_COMPLETED: "bms_autosar.generate.completed",
			GENERATE_FAILED: "bms_autosar.generate.failed",
			KNOWLEDGE_REFRESH_STARTED: "bms_autosar.knowledge_refresh.started",
			KNOWLEDGE_REFRESH_COMPLETED: "bms_autosar.knowledge_refresh.completed",
		},
		TASK: {
	...
	public captureBmsAutosarGenerateStarted(ulid: string, componentCount: number): void {
		this.capture({
			event: TelemetryService.EVENTS.BMS_AUTOSAR.GENERATE_STARTED,
			properties: { ulid, component_count: componentCount },
		})
	}

	public captureBmsAutosarGenerateCompleted(
		ulid: string,
		durationMs: number,
		...
		this.capture({
			event: TelemetryService.EVENTS.BMS_AUTOSAR.GENERATE_COMPLETED,
			properties: { ulid, component_count: componentCount, is_batch: isBatch },
		})
		this.recordHistogram(TelemetryService.METRICS.BMS_AUTOSAR.GENERATE_DURATION_MS, durationMs, {
			ulid,
			component_count: String(componentCount),
			is_batch: String(isBatch),
		})
	}

	public captureBmsAutosarGenerateFailed(ulid: string, error: string): void {
		this.capture({
			event: TelemetryService.EVENTS.BMS_AUTOSAR.GENERATE_FAILED,
			properties: { ulid, error: error.slice(0, 200) },
		})
	}

	public captureBmsAutosarEmbeddingCacheHit(ulid?: string): void {
		this.recordCounter(TelemetryService.METRICS.BMS_AUTOSAR.EMBEDDING_CACHE_HIT_TOTAL, 1, ulid ? { ulid } : undefined)
	}

	public captureBmsAutosarEmbeddingCacheMiss(ulid?: string): void {
		this.recordCounter(TelemetryService.METRICS.BMS_AUTOSAR.EMBEDDING_CACHE_MISS_TOTAL, 1, ulid ? { ulid } : undefined)
	}

	public captureBmsAutosarRetrievalTfidfFallback(ulid?: string): void {
		this.recordCounter(
			TelemetryService.METRICS.BMS_AUTOSAR.RETRIEVAL_TFIDF_FALLBACK_TOTAL,
			1,
			ulid ? { ulid } : undefined,
		)
	}

	public captureBmsAutosarQualityGateIssues(issueCounts: Record<string, number>, ulid?: string): void {
		for (const [issueType, count] of Object.entries(issueCounts)) {
			if (count > 0) {
				this.recordCounter(TelemetryService.METRICS.BMS_AUTOSAR.QUALITY_GATE_ISSUE_TOTAL, count, {
	...
	public captureBmsAutosarKnowledgeRefreshStarted(ulid: string): void {
		this.capture({
			event: TelemetryService.EVENTS.BMS_AUTOSAR.KNOWLEDGE_REFRESH_STARTED,
			properties: { ulid },
		})
	}

	public captureBmsAutosarKnowledgeRefreshCompleted(ulid: string, durationMs: number): void {
		this.capture({
			event: TelemetryService.EVENTS.BMS_AUTOSAR.KNOWLEDGE_REFRESH_COMPLETED,
			properties: { ulid, duration_ms: durationMs },
		})
		this.recordHistogram(
			TelemetryService.METRICS.BMS_AUTOSAR.KNOWLEDGE_REFRESH_DURATION_MS,
			durationMs,
			{ ulid },
		)
	}
